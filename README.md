...
All hooks (including global hooks) also receive these reserved variables, matching the template variables:

| Variable | Template Equivalent | Description |
|----------|-------------------|-------------|
| `TASKP_SKILL_DIR` | `{{__skill_dir__}}` | Absolute path to the SKILL.md directory |
| `TASKP_CWD` | `{{__cwd__}}` | Current working directory at skill start |
| `TASKP_DATE` | `{{__date__}}` | Date at skill start (`YYYY-MM-DD`) |
| `TASKP_TIMESTAMP` | `{{__timestamp__}}` | Timestamp at skill start (ISO 8601) |
