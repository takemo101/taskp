...
- agent モードで外部 MCP サーバーのツールを LLM に提供できる
- `tools: [mcp:github, mcp:slack/post_message]` のように SKILL.md で宣言
- LLM が GitHub API や Slack API を MCP 経由で直接呼び出せる

### 7. セッション ID によるトレーサビリティ

- スキル実行ごとに一意のセッション ID（`tskp_xxxxx`）を自動発行する
- 実行の識別・追跡に利用: CLI 出力、TUI 画面、フック、テンプレート変数、LLM のシステムプロンプト
- ネストされた子スキル呼び出しは親のセッション ID を継承し、実行ツリー全体を1つの ID で追跡可能
