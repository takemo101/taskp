...
| `--dry-run` | | `boolean` | `false` | 実行計画を表示するが実行しない（フックも実行しない） |
...
agent モードでのスキル実行時、`config.toml` に `[mcp.servers]` が定義されていれば、スキルの `tools` フィールドで `mcp:` プレフィックス付きのツールを使用できる。
...
MCP サーバーへの接続はスキル実行時に遅延的に行われ、実行完了後に自動的にクリーンアップされる。詳細は [MCP クライアント仕様](../docs/MCP-SPEC.md) を参照。
