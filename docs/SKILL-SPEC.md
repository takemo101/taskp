...
フックコマンド内では `{{変数名}}` によるテンプレート変数展開が利用可能。`inputs` で定義したユーザー変数と予約変数（`{{__skill_dir__}}` 等）の両方を使用できる。展開タイミングはフック実行直前（inputs 収集後）。
...
inputs:
  - name: environment
    ...
    choices: [staging, production]
hooks:
  before: "{{__skill_dir__}}/scripts/setup.sh {{environment}}"
  after:
    - "echo 'Deployed to {{environment}}'"
```

未定義の変数を参照した場合はエラーとなる（`before` は実行を中断、`after` / `on_failure` は警告のみ）。

> **Note:** 環境変数（`$TASKP_SKILL_DIR` 等）も引き続き利用可能。テンプレート変数はフック実行前に展開され、環境変数はシェル実行時に展開される。
...
5. マークダウン本文の変数を展開
6. hooks のコマンド文字列の変数を展開（各フェーズの実行直前）
7. 実行（template or agent）
```

> **Note:** hooks の変数展開はスキル hooks（`SKILL.md` の `hooks` フィールド）にのみ適用される。`config.toml` のグローバル hooks にはテンプレート変数展開は適用されない（グローバル hooks はスキル横断で共通のコマンドを実行する用途であり、スキル固有の `inputs` を参照する場面がないため。予約変数は環境変数 `$TASKP_SKILL_DIR` 等で代替可能）。
